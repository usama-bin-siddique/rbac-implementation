<div class="bg-white">
    ...
    @if (session('error') === 'data-deleted')
        <p
            ...
            x-init="setTimeout(() => show = false, 2000)"
            class="text-sm text-red-700"
        >{{ __('Record Not Deleted.') }}</p>
    @endif
    <div class="flex items-center gap-4 p-3 flex justify-end">
        <x-primary-button
            onclick="window.location.href='{{ route('tasks.create') }}'">{{ __('Create Task') }}</x-primary-button>
        ...
        <table class="table-auto w-full">
            ...
            @foreach ($taskData as $task)
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="p-4 flex">
                        <a href="{{ route('tasks.edit', $task->id) }}" class="pr-2">
